...
主要用户流程：连接钱包、领取测试币、查看市场、购买、查看持仓、C2C 转让、结算/退款/领取/押金以及查看
链上回执。
...
- 已结算市场会在市场详情和已终局卡片明确显示获胜结果；作废市场明确显示无获胜结果；
- “结算与作废”页同时列出待结算市场和已终局市场；未领取的获胜持仓会提供直达领取入口；
...
- closeAt 前不能结算；
- 结算成功后状态不可逆；写入期间领取/退款/押金按钮与 Resolve/Void 显示“处理中…”且不可连点；
- 页面不会替 Creator 自动选择结果；
- 市场详情和“已终局市场”卡片明确显示与测试物料表一致的结算结果，并高亮获胜项；
...
| 市场结果                        | 测试钱包            | 操作                                       |
| ------------------------------- | ------------------- | ------------------------------------------ |
| Resolved 且持有 winning outcome | B 或 C              | `领取胜出款`                               |
| Resolved 且满足 early-bird      | 指定钱包            | `领取早鸟奖励`                             |
| Resolved 或创建者作废           | 任意钱包            | 先 `释放押金`，再 `领取押金`（打给创建者） |
| Creator void                    | 有 principal 的钱包 | `退还本金`                                 |
| Timeout void                    | 有 principal 的钱包 | `退还本金`                                 |
| Timeout bonus eligible          | 指定钱包            | `领取超时奖励`                             |
| Timeout 且有本金                | 任意钱包            | `释放押金` 罚没，创建者余额不应增加        |

Timeout 市场到交付方提供的 `resolutionDeadline` 后，任意钱包可先点击
`超时作废`，再由各自钱包领取适用款项。已终局市场可直接从结算页
“已终局市场”进入领取 / 退款 / 押金操作，不必再绕去市场页。

正常结算或创建者手动作废后，押金应退还给创建者：先点「释放押金」，再点「领取押金」。
钱打给市场创建者，调用人不能改收款地址。仅超时弃盘且该盘有本金时，释放押金会罚没并进入超时奖励，
创建者不应因此增加余额。

持有非零获胜份额的钱包应在“我的持仓”和已加载的市场详情看到“胜出款待领取”提示，并可通过
“去领取胜出款”直接进入对应 Vault 的“结算与作废”页面。领取仍需用户在钱包中确认，不得自动发送交易。
...
- 钱只支付给固定持有人，调用人不能修改收款地址；领取押金只打给该盘创建者；
- 不适用或重复领取应明确失败，不产生假成功；
- 获胜份额领取前显示提醒和直达入口，领取并刷新后提醒随非零获胜份额一起消失；
- 领取成功后余额变化与交付方提供的预期一致；
- 正常结算或创建者作废后，创建者可通过释放+领取拿回押金；超时弃盘且有本金时不退押金；
- 页面不会因为一次失败自动重复领取。
