...
已有 Mac 部署栈与云端静态托管的固定更新入口为 `npm run stack:update:public`。完成一次专用发布身份和私有更新配置安装后，该命令会拉取 `origin/main`、构建、同步并逐文件校验云端资源、备份和增量升级原数据库、更新现有服务并核验公网；失败时恢复原服务镜像和页面，保留数据库中新操作。重复执行同一已验证版本不会重启服务。详细安装、恢复和验收边界见 [固定更新流程](../deploy/public-site/README.md)。`stack:up` 仍是 Compose 管理命令，不包含云端发布，不能用它的成功替代公网已生效。
