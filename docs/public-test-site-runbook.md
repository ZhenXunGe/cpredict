...
> 当前 V2 ctUSD 测试站的组件版本、验证范围和链上合约边界见
> [求购、自动撮合与自动领取发布说明](orderbook-autoclaim-release.md)。本手册中的早期 ctUSD/time-v2
> 迁移步骤保留为历史操作参考；执行前须以目标环境当前 Compose、镜像、合约地址和私有配置重新预检。

## 2026-09-23 V2 增量更新检查点

本次网页与两个独立自动化服务从 `fa271445afb0d0b23429ce79e746847e3a2e482b` 构建并在新加坡测试站运行；应用、索引、规则服务和链上 Marketplace 未随之更新。后续更新应先记录每个服务的实际镜像 ID、当前 Compose、私有配置、数据库备份和索引 checkpoint，再只替换本次目标服务。云端发布证据与回退副本位于 `/home/ubuntu/cpredict-migration/review-fixes-20260923-fa27144/`。

替换后核对网页真实 HTTPS 入口、七个业务容器健康、索引 `/readyz` 与 `/v1/sync-status?chainId=421614`、回执 `pending/unresolved` 和两个自动化服务的待处理及未知交易状态。检查订单准备阶段的余额不足提示及撮合候选能否在后续轮次继续处理；浏览器夹具回归不能替代真实钱包交易。网页的金额提示是提交前保护，链上 V2 零金额保护仍需新工厂/新合约发布才能生效。未知发送结果只能查询原交易；回退自动化镜像时保持其持久任务和 nonce 记录，不重发、不清库。新合约地址、市场入口和正式交易验收须另行留证，不能从 Git 提交或静态构建推断。
...
2026-09-15 用户要求将当前各类代付额度提高 20 倍。已启用的 ctUSD 环境每周为 `2000000000000000000` wei（2 ETH），按 Asia/Shanghai 周一 00:00 的 `[start,end)` 自然周计算，1.6 ETH 用于新增交易、0.4 ETH 专留退出。当前市场与历史市场共用供应商项目，通过共享准入账本合并计额，迁移不会重置预算；未启用环境上线前须单独核验其配置与供应商策略。对应 sponsor 字段：
...
上例只是 sponsor 的预算字段片段。ctUSD 周总额为 2 ETH，其中 0.4 ETH 专留退出。每日新增/退出通道各自的项目/账户/主体金额和次数、单方法次数按原值乘 20；每日限制仍按 UTC 重置，与周限额同时生效。供应商端已验收的周上限为 2 ETH，单笔上限为 0.02 ETH；本地单笔 Gas 上限及预留保持 0.005 ETH，网关会拒绝超过该上限的 Gas 参数。

放大总预算时不得同步放大 `sponsor.maxCostPerOperation`：准入会预留整个单笔上限，若它也乘 20，可同时等待最终确认的操作数量不会增加。2026-09-15 的误配置曾将单笔预留改为 0.1 ETH，三笔退出操作即可占用 0.3 ETH，加上此前费用，第四笔会触发周额度不足。修正时只恢复新操作的单笔上限，新旧应用运行配置都需更新并重启；不得修改已登记操作的预留或提前释放未最终确认的负债。无其他占用时，0.4 ETH 退出预算可容纳 80 笔 0.005 ETH 的预留，实际费用和每日次数仍会影响可用数量。
