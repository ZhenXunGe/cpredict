...
- claims 与 matching 独立 signer/进程，历史批量领取不会占用撮合 nonce。matching 每 2 秒检查订单事件水位，仅有新事件或满 30 秒维护周期时读取链上订单；同一批次同一市场的终态仅查询一次。claims 无任务每 30 秒补扫，使用账本修订号判断是否需要唤醒休眠账户，并在事实未变化时复用权益账户与市场目录，避免每次全表扫描；两者有在途交易时均每 2 秒查询。
...
- 恶意 ERC1155 接收者可以拒绝收货导致最佳买单成交回滚；资金保持原状，不跳过最优价或更换收款人。最佳买单可在到期后由第三方释放，但到期卖单若拒绝接收返还份额，`releaseOrder` 也会回滚，可能持续阻塞该市场和结果选项的订单簿，不能把“到期”当作自动恢复保证。其他市场和结果选项的订单簿独立；已部署 V2 合约无法原位修复。匹配或释放持续失败应告警，并在新版合约设计中单独解决；详见 `docs/security-hardening-20260924.md`。
