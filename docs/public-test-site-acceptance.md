...
## 2026-09-10 main 部署分支交付

用户确认另一台机器从 `main` 更新部署。本次正常合并 `main` 的 `0f45547` 与用户站 `dev` 的 `6e7e76c`，保留双方历史及部署 Factory／运行包挂载校验；部署来源统一为 `origin/main`。交付提交以包含本节的合并提交为准。

相对已验证的 `6e7e76c`，本次集成增加原有 main 部署校验和其测试，更新部署说明并重新生成源码清单；业务代码、依赖锁文件、账户配置和合约产物保持不变。既有业务、数据库、浏览器和容量证据继续按原范围使用，合并后的编排检查及生成物证据单独保存在 `reports/generated/public-site/main-merge-acceptance.json`。合入 main 不代表已执行远端部署或真实钱包验收。

合并后服务严格编译、73/73 项编排测试（0 失败、0 跳过）、生成物与源码摘要检查及秘密扫描通过，覆盖 main 原有部署校验和 ctUSD 单环境配置。对应记录为 `main-merge-stack.log`、`main-merge-artifacts.log`、`main-merge-secrets.log`；本节不会把既有浏览器／数据库结果重复算成新的实测。

## 2026-09-10 继续实施与当前验收范围

从已提交 `aa9875d` 继续原地升级。下方 2026-09-09 的运行记录保留为历史证据，不代表本轮改动或最终依赖树已验收。

用户最新确认本轮先完成现有 ctUSD 的 PC 全流程，**独立 USDC 部署与验收后移**。iOS、Android、移动钱包实机和移动网络性能均**暂缓／尚未验收**，不作为本轮交付前置条件。PC 的邮箱／Google 内嵌钱包、MetaMask／Rabby、ctUSD 真实零 ETH 代付、资金进出及独立恢复仍须完整实测。先交付本地源码提交，由用户在另一台机器更新部署，再联调实际钱包；不要求先授予服务器控制权。已有三个测试角色私钥，无需再次提供；程序化交易与钱包 UI 分开留证。

本轮实现先在 `dev` 形成 `6e7e76c`，基线为 `aa9875d0b37285248d0d06718affefead8862ddb`，现按上节合入 `main` 交付。以下证据对应该实现的文件和锁定依赖，不能视为基线 HEAD 自身的通过证明。本地报告保留文件摘要、受影响的增量回归及交付绑定；源码提交不等于另一台机器已取得或部署代码。

| 本轮检查 | 结果与范围 | 本地证据（相对 `reports/generated/public-site/`） |
| --- | --- | --- |
| 最终锁文件全新安装 | 一次性目录 `npm ci --ignore-scripts`、严格依赖类型、补丁 5/5、服务构建全部通过；包括 `jayson@4.1.3` 约束 | `final-fresh-install.log`、`final-fresh-types.log`、`final-fresh-patches.log`、`final-fresh-offchain.log` |
| SDK 声明/兼容 | 4 个固定包仅修改声明，校验版本/integrity/运行字节；补丁 5/5、依赖行为 5/5 | `patches-current.log`、`dependencies-current.log` |
| 全量 Vitest | 349 通过，0 失败，31 项数据库测试在此命令中跳过；另有实际 PG 通道 | `unit-final.json`、`unit-final.log` |
| PostgreSQL 联合通道 | 全部 8 文件、31/31 用例实际通过，0 跳过；专用 22 项为其中子集 | `all-postgres.json`、`postgres-ci-gate.log` |
| 原地升级与恢复 | 7 项实际一次性 PostgreSQL 验证：保留旧数据、增量/重复迁移、权限/隔离、未知操作、校验和拒绝、pg_dump/restore | `postgres-ci-gate.log` |
| 编排与入口 | `6e7e76c` 交付时编排工具 72/72；本地执行 5 个镜像选择的编译入口，正确进入配置验证；不是容器启动证明。合并 main 后新增原有部署校验测试，73/73 见上节 | `ctusd-delivery-stack.log`、`entrypoints-current.log` |
| ctUSD 单环境交付回归 | 单环境 CLI 原先强制两份配置，新增回归先失败；修复后仅一份 ctUSD 即可生成公开配置。公开文件在严格 umask 下仍为 0644，不含私有运行字段；拒绝覆盖、空输入和重复环境，保留可选双环境校验。服务严格编译及契约检查通过 | `ctusd-delivery-stack.log`、`ctusd-delivery-offchain.log`、`ctusd-delivery-contracts.log` |
| 浏览器持续回归 | 34/34，PC 17、窄视口 17，0 重试；实际构建入口/Demo/许可页及不会签名的业务夹具。CI 已配置，未远端执行 | `browser-delivery.log`、`browser-regression.json` |
| PC 加载测量 | 三次冷加载中位值：未配置入口 JS 3,142,894 → 895,334 字节，页面可见 174.5 → 128.9 ms；仅本地 Chrome，此时间不代表实际域名/登录/首笔交易 | `pc-build-before-split.json`、`pc-build-delivery.json` |
| 容量 | 正式本地读取容量通过：50 虚拟读者、30 分钟、18,046 次请求，约 9.97 次/秒（含收尾）；p95 16.96 ms、p99 71.75 ms、最大 3.53 秒；1 次失败，错误率 0.00554%；183 次采样，队列最大 0，未发现持续内存增长。SUT 输入摘要无变化 | `capacity-1800-1789028165294/result.json`、`k6.log` |
| 许可副本 | 构建附供应商声明和随包许可，740 条实际安装生产依赖记录；标准许可按条款使用，独立副本补充不阻塞本地提交。实际未知声明和自定义条件继续核对 | `dist/user-site/third-party/inventory.json`、[许可核对](public-test-site-license-review.md) |

供应商管理读取、内部指标和只读反馈已实现，28 项聚焦检查通过；真实金额/策略字段仍等待管理 API 脱敏响应，该输入只阻塞对应报表接入。当前机器没有 Docker/nginx 运行时，容器完整启动、实际 HTTPS、供应商策略正反例、PC 两类真实钱包在 ctUSD 的交易和独立退出均尚未验收；部署后继续验证。USDC 与移动实机为后移项。本轮本地验证没有远端升级、链上广播、公开发布或购买服务。

容量数据来自 100 个市场、50 个账户、10,400 条确定性原始事件和 5,300 条财务事实。负载期间此开发机也执行了构建和浏览器检查。1 次失败原因未从当前汇总日志定位，保留该记录；达到约定低于 1% 的门槛不等于零错误。API 进程 RSS 峰值约 181 MiB，样本线性趋势下降，heap 趋势约 +6.1 KiB/分钟；不包含 PostgreSQL/k6 进程内存。该结果不替代目标主机、RPC、登录、恢复和真实代付容量。

## 2026-09-09 历史证据（保留，不代表本轮最终源码）
...
此前实现在 `dev` 分支基线 `8d35cfa7fc5c6db12381c6d827b6c02cc316e202` 之后形成，测试证明对应文件摘要，不是该基线 HEAD 自身的通过证明。源码文件清单由 `manifests/source-manifest.json` 记录；本地报告、构建摘要与交付提交绑定在 `reports/generated/public-site/acceptance.json`，交付提交以 Git 历史为准。9 月 10 日 `aa9875d` 提交准备仅更新交付文档和对应生成清单，保留前一日运行证据的日期；本轮后续应用改动证据见本文开头。实际部署验收必须重新绑定配置与最终版本。
...
| B3 | 市场、资产、持仓权益、历史、创作者、排行、报表、帮助和反馈页面；本地桌面/手机视口检查 | 本轮 PC 两类真实钱包完整流程；iOS Safari、Android Chrome 与移动钱包实机暂缓，尚未验收 |
