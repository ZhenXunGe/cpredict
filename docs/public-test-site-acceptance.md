...
本轮源码位于 `dev`，基线为 `aa9875d0b37285248d0d06718affefead8862ddb`；交付版本以包含本节的 Git 提交为准。以下证据对应本轮文件和锁定依赖，不能视为基线 HEAD 自身的通过证明。本地报告保留文件摘要、受影响的增量回归及交付绑定；源码提交不等于另一台机器已取得或部署代码。
...
| 编排与入口 | 最新编排工具 72/72；本地执行 5 个镜像选择的编译入口，正确进入配置验证；不是容器启动证明 | `ctusd-delivery-stack.log`、`entrypoints-current.log` |
