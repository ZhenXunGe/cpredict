...
const manifest = {
  ...
  bootstrap: { factoryActivationFingerprint: identity.factoryActivationFingerprint },
...
test("canary inspection binds chain, three accounts, adapter and deployment", () => {
  ...
  assert.throws(() => validateInspection({ chainId: 1 }, manifest), /Arbitrum Sepolia/);
});

test("start result requires exact pre-timeout inventory and 24-hour seeds", () => {
  const result = {
    steps: REQUIRED_CANARY_STEPS
      .filter((id) => id !== "timeout.deadlineCreatorVoidRejected")
      .map((id) => ({ id })),
    timeoutSeed: {
  ...
  assert.throws(() => validateStartResult(result), /86400/);
});
...
test("receipt-driven resume rejects account, adapter and deployment drift", () => {
  ...
  const completeManifest = { ...manifest, source: { ...manifest.source, sourceManifestSha256: "a".repeat(64) } };
  assert.doesNotThrow(() => validateResumeIdentity(state, completeManifest, state.manifestSha256, inspection));
  assert.throws(
    () => validateResumeIdentity(state, completeManifest, state.manifestSha256, {
      ...inspection,
      accounts: [...inspection.accounts.slice(0, 2), "0x8888888888888888888888888888888888888888"],
    }),
    /accounts changed/,
  );
  assert.throws(
    () => validateResumeIdentity(state, completeManifest, state.manifestSha256, { ...inspection, adapterSha256: "b".repeat(64) }),
    /adapter changed/,
  );
});
...
test("finish readiness uses chain time and preserves resumable FINISHING state", () => {
  ...
  assert.throws(() => validateFinishReadiness(state, { chainTimestamp: 999 }), /too early/);
  assert.doesNotThrow(() => validateFinishReadiness(state, { chainTimestamp: 1_000 }));
  assert.doesNotThrow(() => validateFinishReadiness({ ...state, phase: "FINISHING" }, { chainTimestamp: 1_001 }));
  assert.throws(() => validateFinishReadiness({ ...state, phase: "BLOCKED" }, { chainTimestamp: 1_001 }), /cannot finish/);
});
