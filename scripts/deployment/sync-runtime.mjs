...
import { canonicalJson, readJson, sha256Json } from "./evidence-lib.mjs";
...
export function parseSyncArgs(argv) {
  if (argv.length === 0 || !["candidate", "final"].includes(argv[0]))
    throw new Error(
      "usage: deploy:sync -- candidate|final [options] [--permit2-relay]",
    );
...
export function buildRuntimePackage({
  ...
  deploymentBlock,
  inputSha256,
  permit2RelayEnabled = false,
}) {
  if (deployment.chainId !== CHAIN_ID)
    throw new Error(`deployment chainId must equal ${CHAIN_ID}`);
  if (
    mode === "candidate" &&
    deployment.status !== "BOOTSTRAP_SCHEDULED_NOT_FINAL"
  )
    throw new Error(
      "candidate package requires BOOTSTRAP_SCHEDULED_NOT_FINAL input",
    );
  ...
  if (paymentToken === undefined)
    throw new Error(
      "candidate paymentTokenKind must be canonical-usdc or sandbox-test-token",
    );
  ...
  const addresses = finalMode
    ? Object.fromEntries(
        CONTRACT_KEYS.map((key) => [key, deployment.contracts[key].address]),
      )
  ...
  if (finalMode)
    files["web-demo/deployment/final.json"] = prettyJson(deployment);
  else {
    files["web-demo/deployment/debug-addresses.json"] = prettyJson({
      ...deploymentAddresses,
      status: "DEBUG_NOT_FINALIZED",
      warning:
        "Candidate addresses require live code and wiring checks; this is not final evidence.",
    });
...
export async function writeRuntimePackage(
  pkg,
  { outputRoot = RUNTIME_BOUNDARY, runtimeBoundary = RUNTIME_BOUNDARY } = {},
) {
  ...
  try {
    const existing = await readJson(
      resolve(destination, "package-manifest.json"),
    );
    if (existing.inputSha256 !== pkg.packageManifest.inputSha256)
      throw new Error(
        `${destination}: existing package has a different input SHA-256`,
      );
  ...
  const staging = resolve(
    canonicalRoot,
    `.staging-${pkg.identity}-${process.pid}`,
  );
...
async function main(argv = process.argv.slice(2)) {
  ...
  const localSourceManifestPath = resolve(
    ROOT,
  ...
  if (options.mode === "candidate") {
    if (!options.pending)
      throw new Error("candidate requires --pending <pending.json>");
    deployment = validatePendingManifest(
      await readJson(resolveInput(options.pending)),
    );
    ...
    for (const key of [
      ...
      if (!options[key])
        throw new Error(`final requires --${camelToKebab(key)} <value>`);
    ...
    if (
      canonicalJson(sourceResult.manifestRecords) !==
      canonicalJson(deployment.sourceVerification)
    )
      throw new Error(
        "source verification evidence does not match final manifest records",
      );
...
async function resolveDeploymentBlock(options) {
  if (options.deploymentBlock !== undefined) {
    ...
    if (!Number.isSafeInteger(value))
      throw new Error("deployment block is outside safe integer range");
    return value;
  }
  ...
  if (!broadcastPath) {
    const statePath = resolve(
      ROOT,
      "deployments/arbitrum-sepolia/runtime/state.json",
    );
    try {
  ...
  if (!broadcastPath)
    throw new Error(
      "candidate needs --deployment-block or a Foundry --broadcast receipt file",
    );
...
function candidateChecklist(addresses) {
  return (
    `# Arbitrum Sepolia DEBUG candidate\n\n` +
    ...
    `- [ ] Complete the real three-wallet and outcome-deadline plus frozen-window timeout canary.\n` +
