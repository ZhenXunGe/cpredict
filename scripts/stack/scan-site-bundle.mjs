...
const FORBIDDEN = [
  ...
  ["server-env", /\b(?:CPREDICT_AUTOMATION_[A-Z0-9_]+|ARBITRUM_SEPOLIA_RPC_URL|CPREDICT_RPC_FALLBACKS_JSON|CPREDICT_RPC_PROBE_JSON|CPREDICT_STACK_[A-Z0-9_]*PASSWORD|DEPLOYER_PRIVATE_KEY|ARBISCAN_API_KEY)\b/],
...
export async function scanSiteBundle(root) {
  ...
  if (findings.length > 0)
    throw new Error(`Site bundle contains forbidden server material: ${findings.map(({ path, kind }) => `${kind}:${path}`).join(", ")}`);
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const root = process.argv[2] ?? resolve(import.meta.dirname, "../../dist/user-site");
  scanSiteBundle(root).then((result) => process.stdout.write(`PASS Site bundle ${result.files} files, 0 secret findings\n`)).catch((error) => {
