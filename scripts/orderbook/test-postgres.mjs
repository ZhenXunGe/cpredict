...
function run(file, args, options = {}) {
  ...
  if (result.error || result.status !== 0) {
    writeFileSync(
      resolve(root, "work/task-state/postgres-failure.log"),
      (result.stderr ?? "") + (result.stdout ?? ""),
    );
    throw new Error(
      `${file} failed: ${((result.stderr ?? "") + (result.stdout ?? "") + (result.error?.message ?? "")).slice(-8000)}`,
    );
  }
  return result;
}
...
try {
  ...
  started = true;
  await mkdir(resolve(root, "reports/generated/orderbook"), {
    recursive: true,
  });
  const upgrade = await verifyInPlaceUpgrade({ root, pg, directory, port });
  await writeFile(
    resolve(root, "reports/generated/orderbook/in-place-upgrade.json"),
  ...
  const reportPath = resolve(root, "reports/generated/orderbook/postgres.json");
  run(
  ...
  if (
    ...
    report.numTotalTests !== 38 ||
    report.numPassedTests !== 38
  )
    throw new Error(
      "public-site PostgreSQL tests must all execute and pass (38 expected)",
    );
  process.stdout.write(
  ...
  const allPath = resolve(
    root,
    "reports/generated/orderbook/all-postgres-vitest.json",
  );
  run(
    resolve(root, "node_modules/.bin/vitest"),
    [
      "run",
      ...allInventory.map((e) => e.path),
      "--maxWorkers=1",
      "--reporter=default",
      "--reporter=json",
      `--outputFile=${allPath}`,
  ...
  const all = validatePostgresIntegrationResult(
    JSON.parse(await readFile(allPath, "utf8")),
    root,
    allInventory,
  );
  await writeFile(
    resolve(root, "reports/generated/orderbook/all-postgres.json"),
