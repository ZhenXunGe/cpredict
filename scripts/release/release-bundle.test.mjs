...
test("source provenance inventory includes root legal/provenance files without self-reference", async () => {
  ...
  for (const path of [
    ...
    "deploy/host/nginx/cpredict.conf.template",
    "deploy/host/reverse-tunnel/cloud-cli.sh.template",
    "deploy/host/reverse-tunnel/macos-cli.sh.template",
    "scripts/stack/render-reverse-tunnel.mjs",
    "deploy/host/systemd/cpredict-backup.timer",
