...
export const MIN_MARKET_DURATION_MINUTES =
  Number(MARKET_CREATION_MINING_BUFFER_SECONDS / SECONDS_PER_MINUTE) + 1;
export { MAX_MARKET_DURATION_MINUTES };

export function buildCreateMarketTimes(
  nowSeconds: bigint,
  ...
  if (
    ...
    throw new RangeError(
      `市场期限必须在 ${MIN_MARKET_DURATION_MINUTES}–${MAX_MARKET_DURATION_MINUTES} 分钟`,
    );
  }
  ...
  if (earlyBirdStart >= closeAt)
    throw new RangeError("市场期限不足以容纳链上确认缓冲");
