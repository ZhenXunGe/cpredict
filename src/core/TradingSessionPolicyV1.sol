...
interface ISessionMarketplace {
    function listings(bytes32 id)
        external
        view
        returns (address, address, uint128, uint128, uint64, uint8, bool);
}
...
contract TradingSessionPolicyV1 {
    ...
    bytes4 private constant FILL =
        bytes4(keccak256("fillListing(bytes32,uint256,uint256,uint256,uint64)"));
    bytes4 private constant CREATE =
        bytes4(keccak256("createListing(address,uint256,uint256,uint256,uint64)"));
        ...
        returns (uint256 amount)
    {
        if (c.length == 4) {
            ...
            if (selector == BUY) {
                ...
                if (dynamicChecks) _listing(bytes32(_word(c[2].data, 0)), account, false);
            } else {
                revert ForbiddenCall();
            }
            ...
            if (
                c[1].target != marketplace || c[1].data.length != 164
                    || _selector(c[1].data) != CREATE
            ) revert ForbiddenCall();
            address market = _address(c[1].data, 0);
            if (dynamicChecks) _market(market);
            if (_word(c[1].data, 2) == 0 || _word(c[1].data, 3) == 0) revert ForbiddenCall();
            if (
            ...
            if (
                a.target == marketplace
                    && (selector == bytes4(keccak256("cancelListing(bytes32)"))
                        || selector == bytes4(keccak256("returnTerminalListing(bytes32)")))
            ) {
                if (a.data.length != 36) revert ForbiddenCall();
                if (dynamicChecks) _listing(bytes32(_word(a.data, 0)), account, true);
    ...
    function _listing(bytes32 id, address account, bool ownerOnly) private view {
        (address market, address seller,,,,, bool active) =
            ISessionMarketplace(marketplace).listings(id);
        _market(market);
        if (!active || (ownerOnly ? seller != account : seller == account)) revert ForbiddenCall();
    }
