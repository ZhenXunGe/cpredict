...
import { TradingSessionPolicyV2 } from "../src/core/TradingSessionPolicyV2.sol";
...
import { OrderbookMarketplaceV2 } from "../src/marketplace/OrderbookMarketplaceV2.sol";
...
/// @notice Deploys a separate V2 orderbook environment (never rewires an existing factory).
/// @dev Deploys V1-compatible market vaults and schedules the one-time factory wiring through the
/// Timelock. @dev Sandbox and debug deployments use a zero delay; formal retains the 1-hour delay.
contract DeployOrderbookArbitrumSepolia is Script {
    ...
    bytes32 internal constant BOOTSTRAP_SALT = keccak256("CPREDICT_ORDERBOOK_V2_BOOTSTRAP");
    ...
    struct Deployment {
        ...
        OrderbookMarketplaceV2 marketplace;
        SponsorshipPaymasterV1 paymaster;
        TradingSessionPolicyV2 tradingSessionPolicy;
    }
    ...
    struct DeploymentInputs {
        ...
        address tradingSessionPaymaster;
        bytes32 tradingSessionPaymasterCodehash;
        ...
        returns (DeploymentInputs memory inputs)
    {
        ...
        inputs.tradingSessionPaymaster = vm.envAddress("TRADING_SESSION_PAYMASTER");
        inputs.tradingSessionPaymasterCodehash = vm.envBytes32("TRADING_SESSION_PAYMASTER_CODEHASH");
    ...
    function _validateExternalDependencies(DeploymentInputs memory inputs) internal view {
        require(inputs.tradingSessionPaymaster.code.length != 0, "session paymaster code missing");
        require(
            inputs.tradingSessionPaymasterCodehash != bytes32(0)
                && inputs.tradingSessionPaymaster.codehash
                    == inputs.tradingSessionPaymasterCodehash,
            "session paymaster codehash mismatch"
        );
        ...
        returns (Deployment memory deployed)
    {
        ...
        deployed.marketplace = new OrderbookMarketplaceV2(
        ...
        deployed.tradingSessionPolicy = new TradingSessionPolicyV2(
            address(deployed.factory),
            address(deployed.marketplace),
            paymentToken,
            address(deployed.bondEscrow),
            address(deployed.feeVault),
            inputs.tradingSessionPaymaster
        );
    }
        ...
        internal
    {
        ...
        vm.serializeBytes32(
            root, "tradingSessionPaymasterCodehash", inputs.tradingSessionPaymasterCodehash
        );
        ...
        vm.writeJson(
            json,
            vm.envOr(
                "CPREDICT_PENDING_MANIFEST",
                string("deployments/arbitrum-sepolia/orderbook-v2/pending.json")
            )
        );
    }
}
